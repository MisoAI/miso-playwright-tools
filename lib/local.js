...
export async function useLocalBuild(page, {
  ...
  await page.route(
    // Playwright passes a URL object to the route predicate; normalize to the
    // href string so `filter` and `mapping` receive the same type.
    url => filter(url.href, context),
    async (route) => {
